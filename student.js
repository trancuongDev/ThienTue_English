...
function _lgSetGrammarCtx(area) {
  LearningGames.setCtx({
    getGrammarQuestions: () => _stGrammarQuestions,
    areaId: area?.id || 'stGrammarGameArea',
    accent: 'green',
    _area: area || null
  });
}
...
function stRenderMatch(area) { _lgSetVocabCtx(area); LearningGames.vocab.match(area); }
function stRenderMatchSetup(area) { _lgSetVocabCtx(area); LearningGames.vocab.matchSetup(area); }
function stSelectMatchCount(n, btn) { LearningGames.vocab.selectMatchCount(n, btn); }
function stStartMatchGame() { LearningGames.vocab.startMatchGame(); }
function stRenderMatchBoard(area, count) { _lgSetVocabCtx(area); LearningGames.vocab.matchBoard(area, count); }
function stMatchClick(type, id) { LearningGames.vocab.matchClick(type, id); }
function stRenderVocabQuiz(area) { _lgSetVocabCtx(area); LearningGames.vocab.vocabQuiz(area); }
function stRenderVocabQuizCard(area) { _lgSetVocabCtx(area); LearningGames.vocab.vocabQuizCard(area); }
function stVocabQuizPrev() { LearningGames.vocab.vocabQuizPrev(); }
function stVocabQuizNext() { LearningGames.vocab.vocabQuizNext(); }
function stVocabQuizFinish() { LearningGames.vocab.vocabQuizFinish(); }
function stCheckVocabQuiz(optIdx, isCorrect) { LearningGames.vocab.checkVocabQuiz(optIdx, isCorrect); }
function stGrammarGetCorrectOption(q) { return LearningGames.grammar.getCorrectOption(q); }
function stGrammarFormatBlank(text) { return LearningGames.grammar.formatBlank(text); }
function stRenderGrammarFillIn(area) { _lgSetGrammarCtx(area); LearningGames.grammar.fillIn(area); }
function stRenderGrammarFillInCard(area) { _lgSetGrammarCtx(area); LearningGames.grammar.fillInCard(area); }
function stCheckGrammarFillIn() { LearningGames.grammar.checkFillIn(); }
function stRenderGrammarMatch(area) { _lgSetGrammarCtx(area); LearningGames.grammar.match(area); }
function stRenderGrammarMatchSetup(area) { _lgSetGrammarCtx(area); LearningGames.grammar.matchSetup(area); }
function stSelectGrammarMatchCount(n, btn) { LearningGames.grammar.selectMatchCount(n, btn); }
function stStartGrammarMatchGame() { LearningGames.grammar.startMatchGame(); }
function stGrammarTruncate(text, max) { return text && text.length > max ? text.slice(0, max - 1) + '…' : (text || ''); }
function stRenderGrammarMatchBoard(area, count) { _lgSetGrammarCtx(area); LearningGames.grammar.matchBoard(area, count); }
function stGrammarMatchClick(type, id) { LearningGames.grammar.matchClick(type, id); }
...
let _stGrammarGame = 'quiz';
let _stGrammarTheoryOpen = false;

function stSetGrammarTheoryOpen(open) {
  _stGrammarTheoryOpen = open;
  const wrap = document.getElementById('stGrammarTheoryWrap');
  const tag = document.getElementById('stGrammarTheoryTag');
  const icon = document.getElementById('stGrammarTheoryTagIcon');
  if (wrap) wrap.style.display = open ? 'block' : 'none';
  if (tag) tag.classList.toggle('open', open);
  if (icon) icon.textContent = open ? '▼' : '▶';
}

function stToggleGrammarTheory() {
  stSetGrammarTheoryOpen(!_stGrammarTheoryOpen);
}

function stSwitchGrammarGame(mode, btn) {
  _stGrammarGame = mode;
  document.querySelectorAll('.st-grammar-tab').forEach(t => t.classList.remove('active'));
  if (btn) btn.classList.add('active');
  const quizWrap = document.getElementById('stGrammarQuizWrap');
  const gameArea = document.getElementById('stGrammarGameArea');
  const startBtn = document.getElementById('stGrammarStartQuizBtn');
  if (quizWrap) quizWrap.style.display = 'none';
  if (gameArea) gameArea.style.display = 'none';
  if (startBtn) startBtn.style.display = 'none';
  if (mode === 'quiz') {
    if (!_stGrammarQuestions.length) {
      if (gameArea) {
        gameArea.style.display = 'block';
        gameArea.innerHTML = stGrammarEmptyMsg('quiz');
      }
    } else {
      stStartGrammarQuiz();
    }
    ...
    if (!_stGrammarQuestions.length) {
      if (gameArea) {
        gameArea.style.display = 'block';
        gameArea.innerHTML = stGrammarEmptyMsg('fillin');
      }
    } else if (gameArea) { gameArea.style.display = 'block'; stRenderGrammarFillIn(gameArea); }
  } else if (mode === 'match') {
    if (!_stGrammarQuestions.length) {
      if (gameArea) {
        gameArea.style.display = 'block';
        gameArea.innerHTML = stGrammarEmptyMsg('match');
      }
    } else if (gameArea) { gameArea.style.display = 'block'; stRenderGrammarMatch(gameArea); }
  }
}

function stUpdateGrammarTabs() {
  const n = _stGrammarQuestions.length;
  const set = (sel, label) => { const el = document.querySelector(sel); if (el) el.textContent = n ? `${label} (${n})` : label; };
  set('.st-grammar-tab[data-game="quiz"]', '❓ Trắc nghiệm');
  set('.st-grammar-tab[data-game="fillin"]', '✏️ Điền chỗ trống');
  set('.st-grammar-tab[data-game="match"]', '🔗 Nối câu');
}

async function stLoadGrammarQuestions(lessonId) {
  const lid = Number(lessonId);
  let res = await db.from('grammar_questions').select('*').eq('lesson_id', lid).order('sort_order').order('created_at');
  if (res.error && /sort_order|column/i.test(res.error.message || '')) {
    res = await db.from('grammar_questions').select('*').eq('lesson_id', lid).order('created_at');
  }
  return res;
}

async function stFetchGrammarQCounts() {
...
function stGrammarEmptyMsg(mode) {
  const base = '<p style="color:var(--muted);padding:2rem;text-align:center;line-height:1.7">Giáo viên chưa thêm câu hỏi quiz cho bài này.<br/><span style="font-size:.82rem">Lý thuyết và câu hỏi trắc nghiệm là <b>2 phần riêng</b> — GV cần thêm câu hỏi A/B/C/D trong mục Quản lý câu hỏi.</span></p>';
...
async function renderStGrammarLessons() {
  ...
  const qCounts = await stFetchGrammarQCounts();
  list.innerHTML = rows.map(l => {
    ...
    <div style="background:var(--card);border:2px solid var(--border);border-radius:16px;padding:1.25rem;box-shadow:var(--shadow);cursor:pointer;transition:all .18s"
      ...
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:.5rem;margin-bottom:.5rem">
        <div style="font-size:1.05rem;font-weight:800;color:#059669">📐 ${l.title}</div>
        <span style="font-size:.72rem;font-weight:700;padding:.2rem .55rem;border-radius:20px;flex-shrink:0;background:${qc ? '#ecfdf5' : '#fef3c7'};color:${qc ? '#047857' : '#92400e'}">❓ ${qc} câu</span>
      </div>
      ...
      <div style="margin-top:.65rem;display:flex;flex-wrap:wrap;gap:.3rem">
        <span style="font-size:.68rem;background:#ecfdf5;color:#047857;padding:.15rem .45rem;border-radius:6px;font-weight:700">📖 Lý thuyết</span>
        <span style="font-size:.68rem;background:#ecfdf5;color:#047857;padding:.15rem .45rem;border-radius:6px;font-weight:700">❓ TN</span>
        <span style="font-size:.68rem;background:#ecfdf5;color:#047857;padding:.15rem .45rem;border-radius:6px;font-weight:700">✏️ Điền</span>
        <span style="font-size:.68rem;background:#ecfdf5;color:#047857;padding:.15rem .45rem;border-radius:6px;font-weight:700">🔗 Nối</span>
      </div>
      <div style="margin-top:.85rem;background:linear-gradient(135deg,#059669,#10b981);color:#fff;border-radius:10px;padding:.5rem;text-align:center;font-size:.85rem;font-weight:700">Học & luyện tập →</div>
    </div>`;
  }).join('');
}
...
async function stOpenGrammarLesson(lessonId, title) {
  ...
  const { data: qs, error: qErr } = await stLoadGrammarQuestions(lessonId);
  if (qErr) {
    console.error('[grammar_questions]', qErr);
    _stGrammarQuestions = [];
  } else {
  ...
  _stGrammarGame = 'quiz';
  _stGrammarAnswers = {};
  stSetGrammarTheoryOpen(false);
  const quizWrap = document.getElementById('stGrammarQuizWrap');
  const startBtn = document.getElementById('stGrammarStartQuizBtn');
  const gameArea = document.getElementById('stGrammarGameArea');
  if (quizWrap) { quizWrap.style.display = 'none'; document.getElementById('stGrammarQuizList').innerHTML = ''; }
  if (startBtn) startBtn.style.display = 'none';
  if (gameArea) { gameArea.style.display = 'none'; gameArea.innerHTML = ''; }
  const defaultGame = _stGrammarQuestions.length ? 'quiz' : 'fillin';
  stSwitchGrammarGame(defaultGame, document.querySelector(`.st-grammar-tab[data-game="${defaultGame}"]`));
}
...
function stStartGrammarQuiz() {
  ...
  document.getElementById('stGrammarGameArea').style.display = 'none';
  ...
  document.getElementById('stGrammarQuizScore').textContent = `0/${_stGrammarQuestions.length} câu · chọn đáp án rồi bấm Nộp bài`;
  document.getElementById('stGrammarQuizList').innerHTML = _stGrammarQuestions.map((q, qi) => `
    <div style="background:var(--card);border:1.5px solid var(--border);border-radius:14px;padding:1rem 1.25rem">
      <div style="font-weight:700;font-size:.9rem;color:var(--text);margin-bottom:.75rem">${qi + 1}. ${esc(q.question)}</div>
      <div style="display:flex;flex-direction:column;gap:.4rem">
        ${labels.map((lbl, li) => {
          ...
          return `<label style="display:flex;align-items:center;gap:.65rem;padding:.5rem .75rem;border:1.5px solid var(--border);border-radius:10px;cursor:pointer;transition:all .18s;background:var(--bg)" id="gqOpt_${qi}_${lbl}">
            ...
            <span><b>${lbl}.</b> ${esc(q[optKey])}</span>
          </label>`;
        }).join('')}
      </div>
